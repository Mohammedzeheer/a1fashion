...
const index = async function (req, res, next) {
  try {
    ...
    res.render('index', { userCount, blockedUser, orderedCount, totalRevenue, shippedCount, admin: true, dashboard: true })
  } catch (error) {
    next()
  }
}
...
const users = async function (req, res, next) {
  try {
    ...
    res.render('users', { totable, admin: true, useractive: true });
  } catch (error) {
    console.log(error)
    res.render('404')
  }
...
const updateuser = async (req, res, next) => {
  try {
    ...
    await usercollection.updateOne({ _id: updateid }, {
      $set: {
        name: req.body.name,
        phonenumber: req.body.phonenumber,
      }
